...
- **Build output:** [`step3-output.json`](step3-output.json) — audit vs. acceptance criterion: **AC NOT met** (2 tools, in-process only; no run invokes the MCP server; idempotent-ledger half met). See [`../build-report.md`](../build-report.md).
...
- Document MCP server exposes **2** tools: `search_docs`, `read_document` (tested at the `handle_request` level, not via a live stdio subprocess)
- `get_issue` / `create_ticket_draft` / `publish_ticket` are named in the fixture schema + prompts but are **not** callable tools — only `TicketLedger.publish()` exists
- **Not wired:** no agent run invokes the MCP server; the fixed graph reads `fixtures/docs/` directly
